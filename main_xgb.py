...
from mlp_preference_model import *
from xgb_preference_model import *
...
def main(
        ...
        model_params={
            ...
            'colsample_bytree': 0.8,     # sampling ratio for columns when constructing trees
    ...
    dataset = get_dataset(dataset_code, random_state, test_size,
                          min_feature_freq, data_features, food_dt_obj_file)

    # TruncatedSVD(n_components=200, random_state=random_state)
    embedder = None
    ...
    if dataset.similarity_df.shape[0] != diverse_data_size:
        ...
        if diversity_sampling == 'cluster_based':
            save_file = log_dir + \
                f"KMedoidsSampling_target_{dataset_code}_{min_feature_freq}_{diverse_n_clusters}.pkl"
        else:
            save_file = log_dir + \
                f"Random_target_{dataset_code}_{random_state}_{min_feature_freq}_{diverse_n_clusters}.pkl"

        if not os.path.exists(save_file):
            if diversity_sampling == 'cluster_based':
                clustering1 = KMedoids(n_clusters=diverse_n_clusters, metric='jaccard',
                                       init='k-medoids++', random_state=42, max_iter=100)
                diversity_strategy_ = ClusterBasedSampling(
                    clustering1, embedder, outlier_proportion=0, random_state=random_state)
            else:
        ...
        init_target_data = diversity_strategy_.get_sample(
            init_unlabeled_data, diverse_data_size)

        dataset.similarity_df = calculate_similarity(
            dataset, init_target_data.index, init_target_data.index, 'jaccard')
    ...
    # ----------------------------------------------------------------------------------------------------------------
    print(">>>>> STEP-2")
    clustering2 = KMedoids(n_clusters=init_sample_size, metric='jaccard',
    ...
    if not os.path.exists(log_dir+save_file):
        ...
        if diversity_sampling == 'cluster_based':
            diversity_strategy = ClusterBasedSampling(
                clustering2, embedder, outlier_proportion=0, random_state=random_state)
        else:
    ...
    if not os.path.exists(log_dir+init_diverse_sample_file):
        init_diverse_sample = diversity_strategy.get_sample(
            init_target_data, init_sample_size, similarity_df=dataset.similarity_df)
    ...
    if augment_init_sample:
        init_target_similarity_df = dataset.similarity_df.loc[
            init_target_data.index, init_target_data.index]
    ...
    model = XGBoostPreferenceModel(**model_params)
...
if __name__ == '__main__':
    ...
    strategy_names = ['Uncertainty', 'MostUncertainCluster',
                      'ClusteredUncertainty', 'UncertaintyClustered']
    user_names = ['user1', 'user2', 'user3', 'sportive_user1', 'unhealthy_user1',
                  'vegan_user2', 'elder_user2', 'sportive_user3', 'unhealthy_user3', 'random']

    for strategy_name in strategy_names:
        for user_name in user_names:
            print(
                f"Running with strategy_name={strategy_name}, user_name={user_name}")
            main(user_name=user_name, strategy_name=strategy_name,
                 dataset_code=args.dataset_code, random_state=args.random_state)
